...
- Saving a link now works from outside the app. `POST /api/v1/articles/save-url` takes `{"url": "…"}` and does exactly what the box in Saved does, so a phone shortcut in the share sheet, a bookmarklet or a line of curl can keep an article without opening Readfine. The article comes back straight away, with the address standing in for a title while the page is fetched in the background, so a client that wants the text asks for the article again a moment later. A link you already have is attached to your Saved list rather than duplicated, and says so with a plain 200 instead of the 201 a new one gets. Taking an article back out of Saved is in the API too, as `{"is_saved": false}` on the article. The endpoint allows the same ten saves a minute as the web form, since each one fetches a page.

- You can now save an article that isn't in any of your feeds, by pasting its link. A new **Saved** entry sits under Archived in the sidebar, with a box at the top of the list to paste a URL into. The page is fetched, run through the same readable extraction as everything else, and shows up as an ordinary article you can read, label, summarize and chat about. It appears in the list straight away, first under its web address and then under its real headline once the text has been pulled in, so you don't have to wait on the fetch or reload anything. A spinner in front of the address says the text is still coming. A row that ends up with nothing at all, not even the description the page gives for itself, keeps a red mark instead, the same one a broken feed gets in the sidebar, so a row left showing an address is never mistaken for one still loading. A page that hands over its title and blurb but no text needs no mark: it reads like any other row. What went wrong, and the retry, are in the article itself, and an article whose fetch failed on something that may pass later says another attempt is scheduled rather than presenting itself as final. If the link is one you already have, it attaches to that article instead of making a copy, and re-extracts it when all you had was a two-sentence excerpt. Your filters run on it, so a rule scoped to "All articles" can label or star it as it arrives; scoring deliberately doesn't, since you already decided this one was worth keeping. A saved article is kept for good and is never purged by retention, until you remove it from Saved yourself, either from the article's ··· menu or the button row under it. A link that can't be fetched at all is refused on the spot, and one that fails later (a paywall, a 404) is still saved, shows what went wrong, and offers a retry.

- Videos from a YouTube feed now show the video, not just its description. Until now a video reached the reader as text, and the video itself only appeared once something went and fetched the page it lives on, which happened when you opened, starred or labelled the article. It is there from the moment the article arrives, with the whole description under it, chapter marks included. Nothing is fetched to do it: a YouTube feed already carries the video and its description, so the 1.4 MB page they were being read from is not requested at all any more, whether or not you ever open the article. Videos already in your list keep whatever they had; new ones arrive this way.

- Chapter marks in a video's description are clickable. A description that lists "0:00 Intro, 3:40 The build" now turns each of those times into a link that jumps the player here to that point, starting it if it isn't playing yet. Nothing else in a description is linked, since it is mostly sponsor and affiliate URLs. Where the page's scripts don't run, a mark still opens the video on its own site at the same moment.

- Videos now play in the reader. An article holding a YouTube or Vimeo video, whether you saved the link yourself or it came embedded in the article, shows the thumbnail with a play button on it, and clicking that starts the player in place instead of sending you off to the video site. No player is loaded until you press play, so an article you only scrolled past cannot have a video service's cookies set on it. The thumbnail is a different matter and worth being straight about: it is loaded from img.youtube.com or, for Vimeo, vumbnail.com, as it has been since videos in articles first showed a thumbnail at all, so opening the article does tell those hosts your address and which video it was. Serving the thumbnail ourselves is a separate change and not part of this one. YouTube is loaded through youtube-nocookie.com, and the caption under the thumbnail says which site the player will come from. Videos in articles saved earlier play too, and where scripts cannot run, the thumbnail stays what it was, a plain link out to the video.

- The article you are reading is now marked in the list. In the three-panel layout the list gave no clue which of its rows was the one open in the reading pane, so after scrolling around, or coming back to the tab, finding your place meant recognising the headline. The open row now carries a blue bar on its left and the same pale fill the sidebar uses for the section you are in. An article you have already read also stops being dimmed while you are on it, which is what going through starred articles is mostly made of. It follows the article however you got to it, including a link straight to an article and the Next button on a phone, and stays put when the row is redrawn. The two-panel layout is unchanged: there the article opens under its own row, which already says which one it is.

### Changed

- Private feeds are now marked in the feed lists. Settings → Feeds and the admin feeds table put a small lock next to the title of a feed that is kept to your account rather than shared with other subscribers on the instance, which until now you could only find out by opening the feed. Public feeds stay unmarked, since that is what almost every feed is. The badge on the feed edit page went gray at the same time: amber means "something needs attention" everywhere else in the app, and a private feed is not that. In the admin table the lock's tooltip talks about the feed's own subscriber, since the feeds listed there belong to other people.
...
- When an AI provider returns nothing, the error now says why. "returned no usable content" was all you got, which covers three different situations needing three different reactions: the model refused the request, it ran out of room before writing anything, or it genuinely replied with nothing. The notice now carries the provider's own stop reason (and, for Gemini, the reason a prompt was blocked), so you can tell whether to reword a prompt, raise a limit, or just retry.
- The help page now covers the reader itself, not just the setup. A new "Getting around the reader" section names the things you would otherwise have to stumble on: the ↻ and ✓ buttons that appear when you hover a feed, folder, label or view in the sidebar (press and hold on a touchscreen), what the ✓ applies to on each of those rows, folding folders and the sidebar away, the ··· menu on an article and what hides in it, and the `/` shortcut for search. New answers explain Saved, sharing an article by link and how to revoke it, what a red marker on a feed means, where the reading stats live, how long articles are kept and what makes one stay for good, and that there is an API. The existing answers grew the parts that were missing: running a filter over articles you already have, and the preferences that shape reading day to day, from typography to whether the list starts on unread.
- Saving an article now points you at the row it landed on. Saved is ordered by publication date, so a link you paste rarely arrives at the top of the list. A video is the usual case: it carries the date it was published, so saving one from a channel you follow could drop it far down the list, and saving looked like it had done nothing. The list now jumps to the article and flashes it for a moment. Articles it passes on the way stay unread, since none of that was reading. When the article is older than everything on the first page and has no row yet, a short note says it is further down.

- The AI error notice in Settings → AI now says which article the failure happened on, with a link straight to it. Until now the notice gave you the message and nothing else, so a failure on one odd article looked the same as a broken API key, and finding the article meant digging through the server log. Errors that belong to no particular article, such as a failed interest profile, show the message alone as before. If retention has since removed the article, the notice keeps the message and drops the link.

### Fixed

- A link you save with a username and password in it no longer leaves that password in the database. Pasting `https://user:pass@host/article` stored the address as it was typed, which put the password into four columns of the saved article (its link, its dedup key and, until the page gave a title of its own, its name in the list), and into the log line of a fetch that failed. The credentials are now split off before anything is written: they are used for the one fetch that follows and then forgotten, so nothing is left behind in the database or in a backup of it. Saving such a link works exactly as before. What changes is later: because the password is not kept anywhere, the Retry button on that article fetches without it and reports the 401 it gets, and you save the link again to try with credentials. Along the way those credentials stopped riding along to wherever a redirect points, and an address that comes back from the page itself is cleaned the same way. This is the last place a password could reach the database in the clear, after the same was done for feeds (further down this list).

- Readfine no longer downloads a page of any size at all. Nothing put a ceiling on how much a fetch could pull in, which mattered little while the addresses being fetched were feeds you had subscribed to once, but saving an article by its link fetches whatever address you paste, as often as you paste one. A link that turns out to point at a disk image or a video file was read into memory whole. Downloads now stop at 10 MB, counted after decompression, so a small response that unpacks into gigabytes is stopped as well. A page over the limit says so and is not retried on a schedule, since it would be the same size next time; the Retry button still works. The body of a redirect is not downloaded at all any more, since only the address it points to was ever wanted. Self-hosters can change the limit with `MAX_FETCH_BYTES` for a source that genuinely needs more.

- The readable extraction revival panel in the admin dashboard no longer reads as if it contradicted itself. Its heading counted the feeds still waiting for a probe, while the table under it listed the feeds a probe had already brought back, so a "2 feeds awaiting a probe" heading over a single row looked like a row had gone missing. Those are two different sets, and the panel now says so: the heading belongs to the table, and the feeds still waiting are named in a sentence of their own below it. A feed could also turn up in that table claiming it was revived by no probe at all, because saving the feed's form clears the revival bookkeeping while the date of the revival stayed behind. The date now goes with the rest of it.

- Feed addresses in the admin no longer show the secrets some of them carry. A feed can authenticate through its own address, either as `https://user:password@host/feed` or with an API token in the query, and the admin feed list and the fetch log printed those addresses in full, link and all. Both now hide the credentials and the value of any parameter that names a secret, while keeping the rest of the address: that is what tells two feeds on the same site apart, so nothing legible is lost. A redacted address is not a working link any more, so it is shown as plain text instead of a dead one. The edit form for a scraping feed also stopped carrying the address in a hidden field. The preview and the AI selector ask the server for it by id, and the server hands it over only for a feed you are subscribed to.

- Saving a link to a video no longer fails with "the site returned a consent or paywall page". A YouTube watch page carries no article text of its own, since the description under the player is drawn by the site's own scripts, so all that could be extracted from it was the footer ("About, Press, Copyright, Contact us"). Having nothing in common with what the page says it is about, that was read as a substitute page and the link was saved as a failed one, with an error describing a wall that was never there. A video link now saves as the video: the thumbnail, a link to it, and the description below, in full rather than the shortened blurb the page advertises to other sites. YouTube (including youtu.be and Shorts) and Vimeo are recognised, and only pages that are a single video, so a channel or a playlist is still read as an ordinary page. The same applies to articles from YouTube feeds, which until now stored that footer as their whole text.

- The counts next to Starred and Archived in the sidebar could stand above the number of articles those views actually open onto. Articles that reach the end of their retention are stripped to a snippet and hidden everywhere they used to be listed, but the two counters were taken straight off your article states without checking, so such an article kept its place in the badge while being absent from the list. It takes an unlikely sequence to happen (an article has to be starred or archived after it was hidden, which is possible from a list that was on screen before the nightly clean-up ran), and once it did, nothing brought the number back down. Both counters now count what the list shows, and so does the number that appears after you mark a whole view as read, which had the same gap on every row of the sidebar.

- Summaries and relevance scores now work on the newest Claude models. Models of the latest generation (Opus 5, Sonnet 5, Fable 5) reason before they answer even when nothing asks them to, and that reasoning is paid for out of the same allowance as the answer itself. Readfine sizes those allowances for the answer alone, and scoring asks only for a single number, so on one of these models the whole allowance went on reasoning and the reply came back with no text in it. Every request to Claude now says that reasoning should be left off, which puts the allowances back to meaning what they say. Models that insist on reasoning anyway, such as Fable 5, are asked again without it and keep working as before. Nothing changes for OpenAI or Gemini, or for the Claude models you were already using.
- The "Test connection" button in Settings → AI no longer passes a model that cannot actually answer. It sent a greeting and checked that the request came back, not that anything was written in reply, so a model failing in the way described above reported a working connection while every summary and score errored out. It now reads the answer, and a model that returns nothing fails the check with the provider's own reason for it.
- Article text no longer runs off the side of the screen, forcing you to scroll sideways to read a sentence. Editors like WordPress often glue words together with non-breaking spaces around bold text, and a phrase such as "Wednesday, Anthropic and AMD announced" then counts as one 38-character word that a browser cannot wrap, so it hangs over the edge of a narrow reading pane. Newly fetched articles now get the longest of these runs broken back into ordinary spaces, keeping the ones that are there on purpose (10 km, 50 %, one-letter prepositions). Articles already stored, and anything else without a break point such as a very long URL, now wrap instead of overflowing.
- A summary that did run out of room now says so, instead of passing itself off as complete. Readfine stored whatever the model had written by the time it hit the limit and showed it like any other summary, so a summary ending mid-sentence looked like the model's own choice of words. Such a summary is still kept, but the "AI summary" heading above it now reads "AI summary · truncated". Regenerating one that comes back complete clears the note.
...
- A saved article no longer ends up pointing at a cookie-consent page. Saving a link follows the redirects and stores the address the content really came from, so a click tracker doesn't become the article's permanent home. Some links, a Google News one among them, redirect to a consent or sign-in page instead of the article, and that address was adopted just the same, so "Open original" and "Retry extraction" both led back to the consent page from then on. A redirect that ends on a different site is now only adopted when the page there says which address it is, which every real article does. Links that resolve through a shortener or a resolver (youtu.be, doi.org, mobile Wikipedia) are unaffected.
- Switching between Settings and Admin on a phone no longer hides the page you landed on. The row of tabs across the top scrolls sideways, and to reach the "Admin →" or "Settings →" link at its end you have to scroll it there, so arriving on Stats or Dashboard left the strip parked at the far end with the current tab out of sight. The active tab was already being scrolled into view on load, but the browser restores the strip's old position a moment later and undid it. The tab is now brought back into view after that, and left alone as soon as you scroll the strip yourself.
- Full-text extraction no longer turns itself off on a feed where it works. Readfine watches whether a feed already delivers whole articles and switches extraction off when it does, since there is nothing left to fetch. That check read the word count stored with each article, but a successful extraction replaces that number with the word count of the page it pulled in, so a feed publishing two-sentence teasers started to look like a full-content feed once you had read a few of its articles, and extraction was switched off at the next fetch. Nothing showed up under readable errors in the admin panel, because nothing had failed. The check now counts the words the feed itself delivered, which no extraction overwrites, so reading articles cannot influence it. The same measurement decides the setting when you subscribe to a feed someone else on the instance already reads, where another subscriber's working extraction used to be enough to leave you without it.
...
- The readable extraction tables in the admin dashboard no longer scroll sideways. Both tables gained a feed column, which pushed them past the width of the page, and the truncation meant to keep the cells short did nothing, because a table sized by its content ignores it. The feed now sits on a second line under the article title, where it costs no width, and the columns have fixed proportions. The title is cut to one line, since it is a link to the article anyway, while the error message wraps and is always shown in full, being the thing you came to read. On a screen too narrow for all of it the table scrolls sideways rather than squeezing the text, the way the fetch errors table below it already did. Applies to the pending table as well as the failures one.

- The article list highlights the row under the mouse again. The row and the list behind it had both ended up white, so in the light theme nothing happened when you moved across the list and there was no sign that a row is a thing you can click. It has been that way since the list panel's background changed colour and the rows kept the highlight picked to contrast with the old one. Rows now use the same faint gray the rest of the app uses for hovering, and the dark theme keeps the shade it already had. Touchscreens deliberately have no highlight, since a tap would leave one row lit up until you tapped elsewhere; that now follows from the device rather than from the width of the window, so a narrow window on a desktop keeps its highlight.

- An article no longer turns out to be the site's notice about advertising. Some sites answer a fetch that carries no consent cookie by redirecting to a consent page, iDNES among them, and since that page comes back with HTTP 200 the text stored as the article was the cookie and advertising notice, filed under the site's own name. Readfine already refused pages like this by measuring the text against the description the page gives for itself, but that needs a description long enough to judge on, and a consent page carries the site's generic one. A fetch that ends on a page holding the address it interrupted, which is how a consent or login wall returns you afterwards, is now recognised from the redirect itself, so it needs no wordlist and works in any language. Such an article says the site returned a consent page and offers "Open original" and Retry, and the link stays the one you saved, so neither button walks back into the wall. Articles from feeds are covered too: a feed whose pages answer this way had the notice stored as the full text of every one of them, and those now fall back to the text the feed itself delivers. Retrying on a schedule is not attempted, because a wall answers the same way every time, while the Retry button still works.

- Adding a feed by filling in the HTTP auth fields no longer fails on a login. The credentials were saved with the feed but not used for the fetch that happens while the feed is being added, so a feed behind HTTP auth answered 401 at the moment of subscribing, and the only way in was to write the credentials into its address instead. Both ways work now.
