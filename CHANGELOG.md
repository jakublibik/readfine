...
- Error messages are no longer cut off after one line. A feed's last error in Settings → Feeds, and the error columns on the admin dashboard and in the fetch log, all clipped the text to a single line, so on a typical fetch failure you got the opening words and nothing else. They now run to two lines before they trim, with a "Show more" link underneath for the rest, which appears only on messages that are actually too long. The tooltip holding the full message is still there, but it only ever worked with a mouse, so on a phone everything past the first line was unreachable. The readable extraction tables had the opposite problem, wrapping without any limit, so one long message could stretch a row halfway down the page; they follow the same two lines now. A two-line error also makes its row taller, and everything in that row now sits in the middle of it rather than hanging off the top. The dashboard's fetch-error table has also lost some of the empty space between a feed's status and its error, so it takes a little less sideways scrolling on a narrower screen.
