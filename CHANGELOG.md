...
- A model that cannot be told to stop reasoning is now turned down when you choose it, rather than failing quietly on every article afterwards. Scoring gives the model ten tokens to answer in, which is all a single number needs, and a model that reasons whether or not it was asked to (Claude Fable 5 is the one that does this today) spends them thinking before it writes anything, so every score came back empty. Nothing said so except the error banner, one article at a time. Saving a fast model now asks the model itself first, and a model that answers a scoring-sized request with nothing is refused with the reason instead of being stored; Verify says the same thing. Only the model's own answer counts against it, so a rate limit, a missing key or a provider having a moment leaves the choice to you. It can still go in the Quality slot, which briefings and Catch me up can be pointed at. On an account where such a model is already saved, its scoring jobs now give up on the first attempt instead of retrying twice more over the next two hours.
...
- One subscriber of a shared feed can no longer change the login the others fetch it with. A feed's HTTP credentials belong to the feed rather than to you, and go out on every subscriber's request, but the feed edit form offered the fields on any private feed, shared or not. Shared private feeds do exist: the release that moved passwords out of feed addresses marked those feeds private without separating their subscribers. On one of those, anyone subscribed could overwrite the password for everybody, or break fetching for everybody by clearing the username, and could see the username another subscriber had set. The fields are now offered only to a feed's sole subscriber, and where they used to be, the form says why and what to do instead: add the address a second time with the credentials and you get a private feed of your own, which is how a credentialed feed has always been created. The API followed the same rule from the other side and refused to put credentials on a feed that wasn't already private, so it could never set them up in the first place; it now allows what the form allows, on a feed you are the only subscriber to, and turns that feed private in the process.

- Turning readable extraction back on for a blocked feed now sticks. Extraction switches itself off after three page fetches in a row come back refused (403), and those refusals stay recorded on the articles that got them. The check for the next streak read the feed's most recent results without asking when they happened, so once extraction was back on, the old refusals were still the newest ones on record: the first new article that failed made three, and extraction went off again on the spot. It made no difference whether you turned it back on yourself or the scheduled re-check did, and the feed got one article to prove itself instead of three. Only what has happened since extraction came back on is counted now, so a feed gets its full three articles, and the same goes for the longer streak that switches off a feed whose pages extract to nothing.
