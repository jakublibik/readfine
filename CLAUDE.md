...
- **FTS index nesoulad (z tiered-retention review #M)**: inline `_FTS_VECTOR` v `services/article.py` (`title+summary+content+readable_content`, bez unaccent) NEodpovídá výrazu GIN indexu z migrace `0023_drop_old_articles_fts_index` (`immutable_unaccent(title)+immutable_unaccent(content)`). Pro použití expression GIN indexu se musí výraz dotazu přesně shodovat s výrazem indexu → search nejspíš jede seq scan (na ~6000 článcích neznatelné, při růstu pomalé). Ověřit přes `EXPLAIN`, pak sjednotit inline výraz s indexem (nebo index přestavět) + vyřešit `immutable_unaccent`.
- **No-op „hide" filtr akce (z tiered-retention review #A)**: filtr akce `hide` nastaví `UserArticleState.is_hidden = True` (`filter_service.py:464`), ale `is_hidden` se **nikde nefiltruje** v read dotazech (`list_articles` ho nevylučuje) → akce dnes nemá žádný efekt. Retence záměrně použila vlastní `Article.trimmed_at` (per-article), ne `is_hidden`. Buď zapojit `is_hidden` do listingu/počtů, nebo akci „hide" odstranit.
