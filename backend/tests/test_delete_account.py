...
def delete_client(mock_db, client):
    ...
    with patch(
        "app.routers.web.settings.profile.cleanup_user_feeds",
        new=AsyncMock(),
    ):
        yield client
...
class TestDeleteAccountValidation:
    ...
    def test_wrong_password_shows_error(self, delete_client):
        with patch("app.routers.web.settings.profile.verify_password", return_value=False):
...
class TestDeleteAccountSuccess:
    ...
    def test_cleanup_feeds_called(self, mock_db, client):
        ...
        with patch("app.routers.web.settings.profile.cleanup_user_feeds", new=cleanup_mock):
            with patch("app.routers.web.settings.profile.verify_password", return_value=True):
...
class TestDeleteAccountErrorHandling:
    ...
    def test_db_error_returns_500_with_inline_message(self, client, mock_db):
        ...
        with patch("app.routers.web.settings.profile.cleanup_user_feeds", new=AsyncMock()):
            with patch("app.routers.web.settings.profile.verify_password", return_value=True):
